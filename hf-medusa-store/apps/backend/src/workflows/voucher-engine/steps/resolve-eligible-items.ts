 ...
 *
 * Wired into `resolveVoucherDiscountWorkflow` (workflows/voucher-engine/
 * resolve-voucher-discount.ts) and exercised against a real seeded Cart in
 * `integration-tests/http/voucher-engine-resolve-workflow.spec.ts` (mixed
 * product+category scope, no-eligible-items). The eligibility LOGIC itself is
 * also unit-tested directly via `resolveEligibleItems`
 * (lib/__tests__/calculate-discount.unit.spec.ts).
 *
