...
| D5  | "within 15% of threshold"        | `threshold × 0.85 ≤ subtotal < threshold`. (Hằng số code: `CR02_THRESHOLD_PCT = 0.15`.)                                                                  |
...
- `threshold_info` chỉ khác `null` khi **CR-02 fire**; ngược lại `null`. Ngưỡng freeship Phase-1 = `FREE_SHIPPING_THRESHOLD = 7.000.000₫` (fallback hằng số).
