 ...
 * tasks 3.3.14, 3.8.4).
 *
 * Runs AFTER VoucherEngine's own `LineItemAdjustment` rows have been written
 * to the Cart (Decision-4 carrier rewrite — `steps/create-voucher-adjustments.ts`).
 * Refetches the Cart and proves the Cart Module's own recomputed totals match
 ...
 * fails, the workflow's compensation chain removes the just-created
 * adjustments (`createVoucherAdjustmentsStep`'s own compensation) so the Cart
 * recomputes to its pre-voucher state — this step performs no compensation of
 * its own because it is read-only.
 *
 * **No more Rule-11/CONFLICT-8 shrink guard.** The old ephemeral-Promotion
 * carrier needed one because it WAS a Promotion and could shrink a coexisting
 * automatic item-level Promotion's own adjustment (shared, value-DESC-ordered
 * `computeActions` recompute). Raw `LineItemAdjustment`s with `code: null` are
 * invisible to that recompute entirely (verified against installed 2.16.0
 * source — see `steps/create-voucher-adjustments.ts`'s header), so an
 * automatic item-level Promotion's own adjustment can never be reduced by
 * VoucherEngine's adjustment in either temporal order. There is nothing left
 * to detect/guard against — `VOUCHER_STACKING_UNSUPPORTED` is no longer
 * thrown anywhere in the apply/revalidate path.
 *
 * **Framework finding (kept from the prior carrier, still true):**
 ...
 * does NOT run this decoration — every computed total field reads back `0`
 * there. This step therefore reads the authoritative total via the Cart
 * module service directly, not `query.graph`.
 */
...
export interface VerifyTotalsInput {
  cart_id: string;
  /** The `LineItemAdjustment` ids VoucherEngine just wrote for this cart (`create-voucher-adjustments.ts`). */
...
export const verifyCartTotalsStep = createStep(
  ...
  async (input: VerifyTotalsInput, { container }) => {
    ...
    // 2. Sum the adjustment(s) VoucherEngine just created, by id — an exact
    //    match, not a proportional/fractional split, since
    //    `splitAmountAcrossEligibleLines` already computed each line's share
    //    as an exact integer VND (largest-remainder method).
    const adjustmentIds = new Set(input.adjustment_ids);
    const appliedAdjustmentAmounts = (cart.items ?? [])
      .flatMap((item) => item.adjustments ?? [])
      .filter((adjustment) => adjustmentIds.has(adjustment.id))
      .map((adjustment) =>
        toInt(
          adjustment.amount,
          "verify-cart-totals.applied_adjustment.amount",
        ),
      );
  ...
  // earlier createVoucherAdjustmentsStep compensation (soft-delete the
  // adjustments) runs instead, so the Cart recomputes to its pre-voucher
  // state (never a stale write-back — Rule 18).
);
