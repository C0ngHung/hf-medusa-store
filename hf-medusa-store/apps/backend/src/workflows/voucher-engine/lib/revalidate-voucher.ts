 ...
 * ONLY V1, V2, V5, V6, deliberately SKIPPING V3/V4/V7:
 *
 ...
 *
 * No V8 (rebuild-decisions.md decision 2, 2026-07-20): removed along with
 * `stackable_with_promotions` — see `lib/validate-voucher.ts`'s header.
 *
 * A separate function (not a modification of the shared full V1–V7
 * `validateVoucher()` in `lib/validate-voucher.ts`, owned by the Day 3 chain)
 * so the apply-time full pipeline and the cart-change subset can never drift
 * from each other by accident — each calls the same individual,
 * independently-tested validator functions (`lib/validators.ts`), just a
 * different subset/order.
 */

import { v1Exists, v2Window, v5MinOrder, v6Scope } from "./validators";
import type { ValidationResult, VoucherValidationContext } from "./types";

/**
 * This subset deliberately never reads `customer_segment` (V7 is skipped —
 * see file header), so callers don't need to resolve it just to revalidate.
 */
export function revalidateVoucherOnCartChange(
  ctx: Omit<VoucherValidationContext, "customer_segment">,
): ValidationResult {
