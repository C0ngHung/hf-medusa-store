...
```bash
# This repository is pnpm-only (AGENTS.md) — never npm, never yarn.
# Run from the INNER hf-medusa-store/ workspace root:
pnpm build             # Does it compile? (this repo has no typecheck script)
pnpm lint              # Clean?
# Run from apps/backend/ — these scripts set TEST_TYPE, never call jest directly:
pnpm test:unit
...
- [ ] `pnpm build` — compiles without errors (the type gate; there is no `typecheck` script)
- [ ] `pnpm lint` — no linting errors
- [ ] `pnpm test:unit` — plus `test:integration:*` when a module service or endpoint changed
...
| ------------------------------------- | -------------------------------- |
| `nextjs-react-expert` → storefront UI | Render in browser, check console |
| `api-patterns` → API changes          | curl endpoints, check responses  |
