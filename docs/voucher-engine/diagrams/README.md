...
> **Migration note:** these diagrams were authored in an earlier design pass before
> this repo's authoritative `docs/SPEC.md` and `docs/TECHNICAL_SOLUTION_DESIGN.md`
> existed. The "Usage Rule" below is historical — in this repo, the equivalents it
> refers to are `docs/TECHNICAL_SOLUTION_DESIGN.md` (solution flow) and `docs/SPEC.md`
> Part B (VoucherEngine spec), not separate `voucher-engine.solution-flow.md` /
> `SPEC.md` files under this folder.
...
## Usage Rule (historical — see migration note above)
