...
- **Đánh dấu rõ ràng "❌ Không có trong SRS"** với những module SRS không đề cập — tuân thủ nguyên tắc _"Không được giả định khi tài liệu chưa mô tả rõ"_.
- Đưa các phần thiếu vào danh sách **cần clarify với khách hàng** (Phần 11).

> **Cập nhật đối chiếu (2026-07-21):** Toàn bộ 21 trang PDF gốc
> (`docs/SRS_SuggestiveSelling_Voucher_v1.0.pdf`, mục 1–10) đã được đọc lại
> từng trang và đối chiếu câu chữ với tài liệu này. Mọi bảng số liệu, error
> message, business rule, workflow step đã xác nhận khớp 1-1 với PDF trừ các
> điểm được đánh dấu rõ bên dưới (mục 2.2 — mâu thuẫn nội tại trong bảng
> scope; mục 2.0 — glossary bổ sung; mục 12.4 — checklist 22 test đầy đủ).
> Từ nay có thể dùng file markdown này làm nguồn tham chiếu chính cho
> VoucherEngine/SuggestiveSelling mà không cần quét lại PDF; chỉ quay lại PDF
> khi nghi ngờ có sai lệch hoặc khi SRS có bản cập nhật mới (v1.1+).
...
**VOUCH-003 — Discount Stacking & Conflict Resolution** được SRS gọi là _"business rule phức tạp nhất hệ thống"_. Chuỗi tính toán bắt buộc:
...
- **Admin**: cấu hình rule/voucher qua API (SRS ghi rõ _"API only"_ — không có admin UI trong scope).
...
## 2.0 Bảng thuật ngữ (SRS §1.3 Definitions — đầy đủ 10 mục)

| Thuật ngữ                | Định nghĩa (dịch sát nguyên văn SRS)                                                                                               |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------- |
| Suggestive Selling       | Gợi ý sản phẩm bổ trợ hoặc giá trị cao hơn dựa trên sản phẩm khách đang xem/đã có trong giỏ, nhằm tăng giá trị đơn hàng trung bình |
| Product-Level Suggestion | Gợi ý hiển thị trên trang chi tiết sản phẩm (vd xem vợt → gợi ý cước, cán, bao đi kèm)                                             |
| Cart-Level Suggestion    | Gợi ý hiển thị trên trang giỏ hàng, dựa trên tổng hợp nội dung giỏ (vd giỏ có vợt nhưng không có cước → gợi ý cước)                |
| Voucher                  | Mã giảm giá số áp tại checkout, có rule validate cấu hình được (min order, scope, hạn dùng, giới hạn lượt dùng)                    |
| Discount Cap             | Trần tổng giảm giá (tính theo % subtotal giỏ) cho phép từ MỌI nguồn giảm cộng lại (khuyến mãi item + voucher)                      |
| Stacking                 | Việc một voucher có được cộng dồn với các discount đang active khác hay không (khuyến mãi item-level, voucher khác)                |
| MedusaJS Module          | Đơn vị domain độc lập, tự quản lý data model, service và API route riêng                                                           |
| MedusaJS Workflow        | Cơ chế điều phối cho các thao tác nghiệp vụ nhiều bước, có khả năng compensate (rollback)                                          |
| MedusaJS Subscriber      | Handler theo sự kiện, phản ứng với domain event (vd `cart.updated`)                                                                |
| Link Module              | Cơ chế MedusaJS tạo quan hệ entity xuyên module mà không ràng buộc chặt (không FK trực tiếp)                                       |
...
6. **CMS và admin panel để quản lý rule (chỉ API)** — xem mâu thuẫn nội tại ngay dưới đây

> ⚠️ **Mâu thuẫn nội tại của chính SRS (bổ sung 2026-07-21, đối chiếu lại
> bảng gốc trang 2 của PDF ô-theo-ô):** bảng scope gốc xếp dòng **"CMS and
> admin panel for rule management (API only)"** vào cột **Out of Scope**,
> đối xứng theo hàng với dòng In-Scope "MedusaJS Module/Workflow/Subscriber
> implementation patterns" — nghĩa là đọc literal theo bảng, quản trị rule
> bị liệt vào NGOÀI phạm vi. Bản phân tích phiên bản trước (07/07/2026) từng
> xếp mục này vào In Scope (mục 6 cũ) vì hợp lý hơn với phần thân tài liệu:
> **SRS §6 (API Contracts) lại định nghĩa đầy đủ, chi tiết các endpoint
> admin** (`POST/PUT/DELETE /admin/suggestion-rules`, `POST /admin/vouchers`,
> `GET /admin/vouchers/:id/analytics`) — nếu quản trị rule thực sự ngoài
> phạm vi thì các endpoint này không có lý do xuất hiện trong SRS. Đây là
> **mâu thuẫn nội tại thật sự của tài liệu nguồn**, không phải lỗi diễn giải
> của bản phân tích — đã đưa vào danh sách cần hỏi khách hàng (mục 11.3,
> điểm A6 mới). Cách hiểu vận hành đang dùng cho code hiện tại: quản trị
> rule/voucher **qua API là in-scope** (khớp §6 — codebase đã implement đầy
> đủ các route này), chỉ riêng **UI/CMS admin dashboard** mới thực sự
> out-of-scope; đây là cách đọc dung hòa 2 phần mâu thuẫn, không phải nguyên
> văn bảng scope.
...
| Quyền hạn | Chỉ truy cập dữ liệu của chính mình (SEC-04: _"Customer-facing APIs are scoped to the authenticated customer's data only"_)                                                                                                                                      |
...
| Chức năng | CRUD suggestion rule (POST/PUT/DELETE `/admin/suggestion-rules`); tạo voucher (POST `/admin/vouchers`); xem analytics voucher (GET `/admin/vouchers/:id/analytics`); quản lý DiscountCapConfig (SRS §5.2: _"Managed via admin API"_, endpoint cụ thể **không được liệt kê** — cần bổ sung) |
...
| **Edge Cases**     | Hết hàng giữa lúc render và lúc tap → API trả **409** với message _"{Product} just went out of stock. We've updated your suggestions."_ và frontend refresh section gợi ý                                |
...
| **Business Rules**   | **Chỉ 1 voucher active tại một thời điểm.** Áp voucher mới thay thế voucher cũ, có xác nhận: _"Replace current voucher {code}?"_                                                                                                    |
...
_Happy path:_ Vợt 4.500.000 (promo 20% → giảm 900.000) + Cước gợi ý 200.000 (không promo). Voucher SAVE10 (10% toàn giỏ, không cap riêng):

- Post-promo subtotal = 3.800.000 → voucher = 380.000
- Tổng giảm = 1.280.000 / 4.700.000 = 27,2% < 50% ✅
- **Khách trả: 3.420.000₫**

_Cap exceeded:_ Vợt 4.500.000 (promo 40% → giảm 1.800.000) + Cước gợi ý 200.000 (promo 30% → giảm 60.000). Voucher MEGA20 (20%):
...
- **Khách trả: 2.350.000₫**; UI hiển thị: _"Voucher discount adjusted from 568,000₫ to 490,000₫ due to maximum 50% discount policy"_

#### [VOUCH-004] Remove Voucher — Must Have

- Tap "X" trên tag voucher → (a) đảo ngược discount ngay, (b) tổng giỏ về mức trước voucher, (c) **usage count KHÔNG tăng** (chỉ tăng khi đặt hàng thành công), (d) toast _"Voucher {code} removed"_.
...
Cart đổi sau khi voucher đã áp → re-validate. Fail → tự gỡ voucher + notification _"Voucher {code} removed — {reason}"_. Ba tình huống cụ thể:

- (a) Bỏ item → giỏ tụt dưới min*order_value → *"Cart no longer meets minimum {amount}"\_
- (b) Bỏ hết item thuộc category áp dụng của voucher → _"No eligible items remaining in cart"_
...
| EC-10 | Brute-force mã voucher                                                                | Rate limit 5 lần fail / khách / cửa sổ 15 phút → 429 _"Too many attempts..."_; log IP + customer_id                         | Must    |
...
| **Many-to-Many** | `suggestion_rule` ↔ `product` (qua bảng trung gian suggestion*rule_item + Link Module); `voucher_config` ↔ `category`/`product` (qua mảng uuid[] `applicable*\*` — dạng M-N phi chuẩn hóa, chấp nhận theo SRS)                                                                                                                                            |
...
> **Đối chiếu với code hiện tại (bổ sung 2026-07-21):** SRS §6.2 ghi literal
> `GET /store/customer/vouchers` (số ít, không `/me`). Backend hiện implement
> tại `GET /store/customers/me/vouchers` (theo convention chuẩn Medusa —
> `/customers/me/...` cho tài nguyên của khách đã đăng nhập). Đây chỉ là
> khác biệt đặt tên route, không phải sai lệch hành vi — response shape vẫn
> đúng theo SRS. Không cần sửa lại route để khớp literal SRS trừ khi khách
> hàng yêu cầu path chính xác cho tích hợp bên ngoài.
...
**Nguyên tắc bắt buộc từ SRS:** hai custom module giao tiếp với built-in module qua **Link Module + event-driven subscriber**, _"no direct database coupling between modules"_. VoucherEngine đọc SuggestiveSelling chỉ để biết item gợi ý có promotion riêng hay không (input cho stacking).
...
**Tuần 1 — Nền tảng + học Medusa v2:** dựng môi trường (Postgres, Redis, Medusa v2); cả team nắm 4 primitive: Module (isolated data model + service), Workflow (step + compensation), Subscriber (event), Link Module (quan hệ cross-module không coupling DB). Scaffold 2 module, seed data cầu lông, CI + khung test. _Milestone: `medusa develop` chạy, 2 module đăng ký thành công._

**Tuần 2 — [A] VoucherConfig + validate V1–V4 · [B] Data model suggestion + Tier 1:** A: migration voucher*config/usage_log, normalize + lookup + expiry + usage check (Redis atomic). B: migration 4 bảng suggestion, Link tới Product, CRUD admin rule, đánh giá Tier 1 manual. \_Milestone: T-VOUCH-02/03/04 xanh; admin tạo được rule.*

**Tuần 3 — [A] V5–V8 + apply/remove + i18n · [B] Tier 2 + filters:** A: validate cart/segment, calculateDiscount + max*discount_amount, attach/remove với compensation, message vi/en. B: category complement backfill, 4 filter SUGG-002, rank & limit, enrich pricing. \_Milestone: T-VOUCH-01/05/06/10 + T-SUGG-01→04 xanh.*

**Tuần 4 — [A] Stacking + Global Cap ⭐ · [B] Cache + one-tap add:** A: 6 rule VOUCH-003, DiscountCapConfig + admin API, enforceGlobalCap, cap*explanation, sàn 1 VND + warning log; verify đúng fixture SRS. B: Redis cache + invalidation theo rule change, API add từ gợi ý + re-check stock 409. \_Milestone: T-VOUCH-07/08/09 + T-SUGG-06 xanh.*

**Tuần 5 — [A] Revalidation + concurrency + rate limit · [B] Cart-level rules:** A: workflow 7.3, subscriber, optimistic locking, 5-fail/15-phút + cooldown 30 phút. B: CR-01→CR-04, top-3 unique, threshold*info, ẩn section rỗng. \_Milestone: T-VOUCH-11/12 + T-SUGG-07/08 xanh.*

**Tuần 6 — Tích hợp chéo + frontend wiring:** backend: EC-01/02/03/05/08 end-to-end; SUGG-005 invalidate ngay trên cart change. Frontend: 3 vùng UI (§7.2) nối API thật, toast/undo, cap banner, replace confirm. _Milestone: T-SUGG-09 xanh; demo flow đầy đủ product→cart→checkout→cap._

**Tuần 7 — Analytics + admin còn thiếu:** SuggestionEvent tracking 4 action + partition; voucher analytics endpoint; các API bổ sung mục 6.5 (sau khi khách duyệt); EC-09 xác nhận hành vi stale. _Milestone: T-SUGG-10 xanh; dashboard số liệu thô truy vấn được._

**Tuần 8 — Hardening + UAT:** load test đạt p95 §9.1, đo cache hit >85%, security review SEC-01→04, chạy toàn bộ checklist §10 (22 test), fix bug, tài liệu bàn giao. _Milestone: 22/22 acceptance test pass, sẵn sàng release._
...
6. **CMS/admin panel scope (bổ sung 2026-07-21):** bảng scope gốc (§1.2) xếp "CMS and admin panel for rule management (API only)" vào cột Out of Scope, nhưng §6 (API Contracts) định nghĩa đầy đủ các endpoint admin (`/admin/suggestion-rules`, `/admin/vouchers`, `/admin/vouchers/:id/analytics`). Cần chốt: quản trị qua API có in-scope không, hay toàn bộ (kể cả API) đều out-of-scope như bảng ghi literal? Xem phân tích đầy đủ ở mục 2.2.

**Nhóm B — Thiếu định nghĩa:**

7. **"Assigned store"** (SUGG-002b, query `store_id`): khách được gán store thế nào khi store locator out of scope? Guest thì sao?
8. **Nơi lưu dismiss-in-session:** client-side hay server-side? Ảnh hưởng trực tiếp thiết kế cache.
9. **V5 min_order_value:** so với subtotal **gốc** hay **hậu-item-promotion**? Chênh lệch có thể quyết định voucher hợp lệ hay không.
10. **Cap 50% có tính phí ship không?** "Original cart subtotal" được hiểu là hàng hóa thuần — cần xác nhận.
11. **Quy ước làm tròn:** integer arithmetic + phép % → làm tròn xuống/lên/half-up? Ảnh hưởng từng đồng, đặc biệt khi cap.
12. **Tích hợp CRM "My Vouchers":** cơ chế gán (API? import?), bảng liên kết customer↔voucher không có trong data model §5.
13. **Nguồn "top-selling"** cho Tier 2 và CR-01: cửa sổ thời gian nào, tính theo số lượng hay doanh thu, lấy từ đâu (Order module? analytics ngoài?).
14. **Nơi lưu category complement mapping** (Rackets→Strings...): hardcode, bảng config hay CartSuggestionCondition?
15. **Guest checkout:** per_user_limit, lịch sử mua 30 ngày, "My Vouchers" đều cần customer_id — hành vi với guest chưa đặc tả (auth out of scope).
16. **Endpoint DiscountCapConfig + GET/PUT/DELETE voucher + GET list rules:** thiếu trong §6 (đã nêu mục 6.5).
17. **EC-08 tier promo "Spend 5M":** ngưỡng 5M tính trên subtotal gốc hay hậu-discount? Quyết định có xảy ra vòng lặp recalculate hay không.
18. **Re-validate voucher tại order placement:** để bịt lỗ hổng usage_limit (mục 11.2) — SRS không yêu cầu, đề xuất bổ sung.
...
### 12.4.1 Bảng đầy đủ SRS §10 — Acceptance Test Checklist (22 test, nguyên văn từ PDF)
...
> **Ghi chú đối chiếu:** đây là bản dịch/giữ nguyên 1-1 từ PDF trang cuối
> (§10), bổ sung 2026-07-21 vì bản phân tích trước chỉ có phụ lục truy vết
> rút gọn (không liệt kê Scenario cho từng test). `.claude/rules/testing.md`
> của repo (mục "Acceptance coverage") đặt mục tiêu ≥20/22 test này pass —
> con số 22 khớp chính xác bảng trên (10 SUGG + 12 VOUCH).
...
_Tài liệu phân tích lập bởi vai trò Solution Architect / BA / Tech Lead / PM, bám sát 100% nội dung SRS v1.0. Mọi điểm SRS không đặc tả đã được đánh dấu ❌ hoặc đưa vào danh sách clarify (mục 11.3) thay vì tự giả định._
