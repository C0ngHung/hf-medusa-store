...
#### This repository (pnpm-only — never npm, never yarn)

Run from the INNER `hf-medusa-store/` workspace root:

1. **Lint/Fix:** `pnpm lint`
2. **Types:** `pnpm build` — there is no `typecheck` script; the build is the type gate
...
1. **Write/Edit Code**
2. **Run Audit:** `pnpm lint` then `pnpm build`
