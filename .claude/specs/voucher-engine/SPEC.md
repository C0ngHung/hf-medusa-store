...
| The discount amount landing in **Cart & Order totals**                                                                                              | **an ephemeral, cart-specific Medusa Promotion** (id in `cart.metadata.voucher`, Decision G) | The native _carrier_ of the capped amount into authoritative `discount_total` / `items.adjustments` (§14.2-A). VoucherEngine computes the capped amount; a fresh per-cart fixed-amount Promotion transports it. `VoucherConfig.promotion_id` is the **canonical/reference** Promotion only (never cart-attached). |
...
Maps SRS §5.2 `VoucherConfig`, minus the `extends Promotion` clause (see §18 CONFLICT-1). Implemented as a standalone model that **references a canonical backing Medusa Promotion** (§14.2-A) via the `promotion_id` text field below — this is the reinterpretation of SRS "extends Promotion": VoucherEngine owns voucher-specific config + audit, while the discount is carried at apply time by a **fresh ephemeral, cart-specific Promotion** (Decision G, §14.2-A) so it lands in authoritative Cart/Order totals. `promotion_id` itself is the canonical/reference promotion (admin/analytics/template), never attached to a cart.
...
| `promotion_id`              | `model.text().nullable()`                                       | **Decision C (approved), re-scoped by Decision G:** id of the **canonical/reference** Medusa **Promotion** (§14.2-A) provisioned by `createVoucherWorkflow` for admin/analytics reference and as the template for apply-time ephemeral promotions — client input is never trusted for this value. Cross-module ref → read-only Link to Promotion (§6), never DB FK. **It is NOT attached to carts and does NOT identify the cart/order adjustment** — the actual adjustment carries the _ephemeral_ promotion's id (from `cart.metadata.voucher` / `order.metadata.voucher`), Decision G. Requires a migration (not present on the currently shipped model) and tests. |
...
> **These §10 formulas are VERIFIED-CORRECT and UNCHANGED by the 2026-07-15 finding.** The Rule-11 break (CONFLICT-8/PD-15) is entirely in the _carrier/transport_ (§14.2-A / Decision G), not in this calculation. `final_voucher_discount` computed here is correct; do not alter §10.1/§10.3 for CONFLICT-8. See the carrier caveat in §10.2 and §18 CONFLICT-8.
...
- item-level promotion discount is **never** reduced by VoucherEngine (Rule 11). **⚠ CONFLICT-8 / PD-15 (2026-07-15):** the §14.2-A / Decision-G ephemeral-fixed-Promotion carrier **verifiably violates this** whenever a percentage-type item/order-level Promotion coexists on the same line(s) — Medusa's `computeActions` processes promotions in `application_method.value DESC` order and recomputes the later (percentage) promotion against a base already reduced by the earlier (fixed voucher) promotion, so the item-promo's own adjustment silently shrinks. This is a **carrier problem, not a §10 calc problem**: the pure calculator's `final_voucher_discount` is correct; the promotion _transport_ corrupts a coexisting percentage promo. The slice is BLOCKED on the carrier decision (PD-15); until resolved, `verifyCartTotalsStep` MUST detect the shrink and fail closed with `VOUCHER_STACKING_UNSUPPORTED` (§18 CONFLICT-8, Decision-G finding, §23.4).
...
This makes Rule 11 ("never reduce item promotions") **detectable** directly against the data: VoucherEngine's _own_ calculation only ever changes the voucher's own adjustment amount, never the others. **⚠ CORRECTION (2026-07-15, CONFLICT-8/PD-15):** the earlier claim that this makes Rule 11 "enforceable" was wrong for the Decision-G carrier. VoucherEngine's calc is Rule-11-clean, but the Medusa promotion _engine_ itself reduces a coexisting **percentage** item/order promotion's adjustment when the ephemeral fixed voucher promotion is attached alongside it (value-DESC processing order + shared `appliedPromotionsMap`; see Decision-G finding + §18 CONFLICT-8). The per-line pre-apply `item_promotion_discount` `loadCartContextStep` already computes is therefore the **baseline** `verifyCartTotalsStep` must compare against to _detect_ (and reject via `VOUCHER_STACKING_UNSUPPORTED`) — not something the carrier can _guarantee_.
...
3. `revalidateStep` — re-run the cart-change validation subset **V1, V2, V5, V6, V8** (§9.2).
   3a. valid → `calculateVoucherDiscountStep` + `enforceGlobalCapStep` → **replace the ephemeral promotion** carrying the new amount: `REMOVE`+delete the old ephemeral promotion, then create+`ADD` a new ephemeral fixed-amount promotion (value = recomputed `final_voucher_discount`) and update `cart.metadata.voucher`; the Cart module recomputes authoritative totals from source. (A promotion's `value` is not mutated in place — replacing the ephemeral record keeps each amount cart-specific, Decision G.) **CONFLICT-8/PD-15 baseline requirement:** the `pre_apply_item_promotion_discount` baseline fed to the post-ADD `verifyCartTotalsStep` shrink-guard MUST be the **voucher-free** item-promotion discount — i.e. captured **AFTER** the old ephemeral promotion is REMOVEd+deleted (or from a clean recompute with no voucher adjustment present), **never** from a `loadCartContextStep` read taken while the old ephemeral is still attached. Otherwise the baseline is itself already shrunk and the guard would pass on a Rule-11-violating state instead of triggering `VOUCHER_AUTO_REMOVED`. (`loadCartContextStep` only excludes the voucher's _own_ adjustment by `promotion_id`; it does NOT un-shrink a coexisting percentage promo, so the removal ordering is what makes the baseline clean.)
   3b. invalid, OR the shrink-guard detects a coexisting percentage promotion was reduced (a newly-active storewide % sale after the voucher was applied) → `removeVoucherPromotionStep` (+delete ephemeral promotion) + build `VOUCHER_AUTO_REMOVED` reason (stacking-unsupported is an auto-removal reason on the revalidate path, not a hard 500 as on first apply).
...
| ------------- | ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------ |
| PD-01         | Voucher↔Cart association + total representation                    | **RE-OPENED via PD-15 / CONFLICT-8 (2026-07-15) for the percentage-promo-stacking case.** Was RESOLVED (Decision G, ephemeral fixed Promotion via `createPromotionsWorkflow` + `updateCartPromotionsWorkflow ADD`); that carrier is now verified to break Rule 11 whenever a percentage item/order promotion coexists, so the carrier choice is BLOCKED under PD-15 (credit-lines re-elevated from "rejected" to leading candidate, pending tax + `discount_total`/analytics sign-off). Metadata-only + shared-promotion-override remain rejected. `[NV #3]` signature still resolved; identity via `order.metadata.voucher` still valid.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | apply/remove promotion steps, §14.2-A                        |
...
| PD-15         | Apply-time discount carrier vs Rule 11 (percentage-promo stacking) | **OPEN — BLOCKS apply/replace/revalidate (2026-07-15, CONFLICT-8).** Decision-G ephemeral fixed Promotion verifiably reduces a coexisting **percentage** item/order promotion (Medusa `value DESC` processing + shared `appliedPromotionsMap`; ordering escape infeasible). Committed now: `verifyCartTotalsStep` must fail closed with `VOUCHER_STACKING_UNSUPPORTED` on any non-voucher-adjustment shrink (§10.2/§23.4) — diagnostic only. Pending decision on the real carrier: **leading candidate `cart.credit_lines`** (verified immune to promotion compounding — not a promotion, never in `computeActions`; reduces `cart.total` `utils/dist/totals/cart/index.js:112`; propagates to order `complete-cart.js:361-367,406`; add/remove via `create/deleteCartCreditLinesWorkflow`) — held for two sign-offs: (i) **tax base** — credit lines subtract AFTER tax (line 112) instead of reducing the taxable base a promotion adjustment reduces (money difference where tax configured; `[NV #17]`); (ii) **product** — credit lines never feed `discount_total`/`items.adjustments`, so the voucher stops appearing as a discount in orders/receipts/analytics. Alternative (redefine §10/Rule 11 to compounding semantics) rejected without business sign-off. | apply/replace/revalidate carrier, §14.2-A, §10.2, Decision G |
...
| #3b             | **STILL OPEN (Decision G residual).** `updatePromotionsWorkflow` / `deletePromotionsWorkflow` input signatures — needed for §11.7 `syncBackingPromotionStep`, §11.8 `deactivateBackingPromotionStep` (canonical-promotion update/deactivate) and for **ephemeral-promotion delete** on remove/revalidate/cleanup (NV#15). Not verified this session. The canonical-promotion sync is cosmetic under Decision G, but the **delete** signature is load-bearing for apply/remove cleanup.                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `@medusajs/core-flows/dist/promotion/workflows/{update,delete}-promotions.d.ts`                               | §11.7, §11.8, §14.2-A |
...
| #15             | **Ephemeral-promotion post-order cleanup timing (Decision G).** On remove/revalidate the ephemeral Promotion is deleted synchronously. For a **completed order**, confirm (a) whether the order's copied `items.adjustments` remain intact if the ephemeral Promotion is later deleted (expected yes — adjustments are copied to the order, not FK-joined; complete-cart copies them), and (b) the correct trigger to delete the now-orphaned ephemeral Promotion **after** `recordVoucherUsageWorkflow` succeeds (e.g. a cleanup step in the redemption workflow or a scheduled job). If not confirmable, leave the ephemeral Promotion in place (harmless, `is_automatic:false`) and reap via a periodic job — do NOT delete before redemption is recorded.                                                                                                                                                                                              | `@medusajs/core-flows` `complete-cart` / `deletePromotionsWorkflow`; requires empirical check                 | §14.2-A, Decision G   |
...
| #17             | **Credit-lines carrier tax + end-to-end viability (PD-15 candidate, 2026-07-15).** Verified so far: `cart.total = subtotal + taxTotal − discountSubtotal − creditLinesTotal` (`@medusajs/utils/dist/totals/cart/index.js:112`) so credit lines reduce total but are subtracted AFTER tax and never feed `discount_total`/`items.adjustments`; created via `createCartCreditLinesWorkflow` `{cart_id,amount,reference,reference_id,metadata}`, propagate to order (`complete-cart.js:361-367,406`). **Still to confirm before adoption:** exact tax-base impact vs a promotion adjustment when tax IS configured (the SRS VND worked examples are tax-free, so impact may be nil for MVP — verify against the store's tax config), whether a manually-created credit line survives cart→payment→order unchanged, and interaction with `verifyCartTotalsStep` (which must switch to comparing `cart.total`, not `discount_total`). Empirical check required. | `@medusajs/core-flows` `create/delete-cart-credit-lines`; `@medusajs/utils/dist/totals/cart` + tax calculator | PD-15, §14.2-A        |
...
4. **Owned types:** `interface VerifyTotalsInput { cart_id: string; promotion_id: string; final_voucher_discount: number; expected_final_cart_total: number; pre_apply_item_promotion_discount: number /* CONFLICT-8/PD-15: baseline Σ non-voucher adjustments observed by loadCartContextStep before ADD */ }`; `interface VerifyTotalsOutput { cart: unknown /* refetched authoritative cart */; verified: true }`.
...
8. **Ordered algorithm:**
   ...
   4. **MANDATORY Rule-11-shrink guard (CONFLICT-8/PD-15).** Recompute the post-apply non-voucher item-promotion discount = `sumInts(cart.items.flatMap(i => i.adjustments).filter(a => a.promotion_id !== input.promotion_id).map(a => toInt(a.amount)))`. **Assert it is `>= input.pre_apply_item_promotion_discount`** (i.e. no pre-existing item/order promotion's adjustment was reduced by attaching the voucher). Any decrease means Medusa's promotion engine reduced a coexisting (typically percentage-type) promotion — a Rule-11 violation. On decrease → throw the **distinct** `VOUCHER_STACKING_UNSUPPORTED` (NOT the generic `VOUCHER_CALCULATION_FAILED`), so the failure is diagnosable. This is a fail-safe, **not** a functional fix (the carrier itself is BLOCKED under PD-15). **Baseline invariant (load-bearing):** `input.pre_apply_item_promotion_discount` MUST be the **voucher-free** item-promotion discount. On first apply this is `loadCartContextStep`'s pre-ADD read (no voucher present). On the revalidate-replace path (§11.3) it MUST be captured **after** the old ephemeral promotion is removed/deleted (or from a clean no-voucher recompute) — otherwise the baseline is itself already shrunk and the guard silently passes on a violating state.
...
14. **Mapped tests:** http-integration `integration-tests/http/apply-voucher.spec.ts` asserts the response cart `total` equals the §10.4 contract (`3_420_000`) and that a deliberately mismatched fixture triggers `VOUCHER_CALCULATION_FAILED` with the cart reverted (no voucher adjustment remains). (T-VOUCH-01; INT-03.) **CONFLICT-8/PD-15 test (required):** a cart with a live **percentage-type** item/order Promotion + a voucher must trigger the Rule-11-shrink guard → `VOUCHER_STACKING_UNSUPPORTED` (distinct from `VOUCHER_CALCULATION_FAILED`), and the cart must be fully reverted (the pre-existing percentage promotion's adjustment restored to its original amount, no voucher adjustment remains). Also assert a **fixed-amount** item Promotion + voucher (under the 50% cap) still succeeds (regression guard proving the block is scoped to percentage promos, not all stacking).
