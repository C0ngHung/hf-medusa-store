...
async function main() {
  ...
  try {
    ...
    emit('ask', 'Safety gate could not read the payload; falling back to a permission prompt.');
    return 0;
  }
  ...
  try {
    ...
    console.error('safety gate warning: Antigravity sent invalid JSON; deferring to the permission prompt.');
  ...
  if (!command) {
    console.error('safety gate warning: no command found in the payload; deferring to the permission prompt.');
  ...
  if (!result.allowed) {
    emit('deny', `Blocked by the workspace safety gate (${result.rule}): ${result.reason}.`);
    return 0;
  }
