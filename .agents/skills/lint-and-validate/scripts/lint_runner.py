...
Supports:
    - Node.js: <pm> run lint, tsc --noEmit (package manager detected from the lockfile)
...
def detect_project_type(project_path: Path) -> dict:
    ...
    if package_json.exists():
        ...
        try:
            ...
            # Detect the package manager from the lockfile and fall back to pnpm.
            if (project_path / "pnpm-lock.yaml").exists():
                ...
                pm = "pnpm"
            runner = "pnpm" if pm == "pnpm" else "npx"
            ...
            if "typescript" in deps or (project_path / "tsconfig.json").exists():
                result["linters"].append({"name": "tsc", "cmd": [runner, "tsc", "--noEmit"]})
...
def run_linter(linter: dict, cwd: Path) -> dict:
    ...
    try:
        ...
        # Windows compatibility for package-manager shims
